...
async function fetchRepoContentFromVersion(fetcher, versionData, {
  ...
  return _objectSpread(_objectSpread({}, appContent), {}, {
    contractAddress,
    version
  });
}
...
function _default(web3, {
  ...
  return {
    ...
    fetchLatestRepoContentForContract: async (repoAddress, codeAddress, options) => {
      const repo = (0, _repo.makeRepoProxy)(repoAddress, web3);
      const fetchVersionData = (0, _overrides.shouldOverrideAppWithLatestVersion)(repoAddress, codeAddress) ? (0, _repo.getRepoLatestVersion)(repo) : (0, _repo.getRepoLatestVersionForContract)(repo, codeAddress);
      return fetchRepoContentFromVersion(fetcher, await fetchVersionData, _objectSpread({
        fetchTimeout
      }, options));
    }
  };
