...
async function getRepoVersionById(repoProxy, versionId) {
  ...
  try {
    contentURIDecoded = (0, _web3Utils.hexToAscii)(contentURI);
  } catch (_err) {
...
async function fetchRepoContentURI(fileFetcher, contentURI, {
  ...
  try {
    ...
    const filesFetch = Promise.all([fileFetcher.fetch(provider, location, 'manifest.json', {
      timeout
  ...
  return _objectSpread(_objectSpread(_objectSpread({}, manifest), artifact), {}, {
    content: {
      provider,
      location
    }
