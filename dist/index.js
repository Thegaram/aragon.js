...
Object.defineProperty(exports, "ensResolve", {
  enumerable: true,
  get: function () {
    return _ens.resolve;
  }
});
Object.defineProperty(exports, "AddressIdentityProvider", {
  enumerable: true,
  get: function () {
    return _identity.AddressIdentityProvider;
  }
});
Object.defineProperty(exports, "getRecommendedGasLimit", {
  enumerable: true,
  get: function () {
    return _transactions.getRecommendedGasLimit;
  }
});
exports.default = exports.detectProvider = void 0;
...
var handlers = _interopRequireWildcard(require("./rpc/handlers"));

var _apps = _interopRequireWildcard(require("./apps"));

var _cache = _interopRequireDefault(require("./cache"));

var _apm = _interopRequireWildcard(require("./core/apm"));

var _repo = require("./core/apm/repo");

var _aragonOS = require("./core/aragonOS");
...
var _ens = _interopRequireWildcard(require("./ens"));
...
var _callscript = require("./utils/callscript");

var _forwarding = require("./utils/forwarding");
...
const detectProvider = () => typeof web3 !== 'undefined' ? web3.currentProvider // eslint-disable-line
...
class Aragon {
  constructor(daoAddress, options = {}) {
    ...
    options = Object.assign(defaultOptions, options); // Set up desired configuration

    (0, _configuration.setConfiguration)(configurationKeys.FORCE_LOCAL_STORAGE, !!(options.cache && options.cache.forceLocalStorage));
    (0, _configuration.setConfiguration)(configurationKeys.SUBSCRIPTION_EVENT_DELAY, Number.isFinite(options.events && options.events.subscriptionEventDelay) ? options.events.subscriptionEventDelay : 0); // Set up Web3

    this.web3 = new _web.default(options.provider); // Set up ENS

    this.ens = (0, _ens.default)(options.provider, options.apm.ensRegistryAddress); // Set up APM utilities

    const {
      ipfs: apmIpfsOptions = {}
    } = options.apm;
    this.apm = (0, _apm.default)(this.web3, {
      fetchTimeout: apmIpfsOptions.fetchTimeout,
      ipfsGateway: apmIpfsOptions.gateway
    }); // Set up the kernel proxy

    this.kernelProxy = (0, _utils.makeProxy)(daoAddress, 'Kernel', this.web3); // Set up cache

    this.cache = new _cache.default(daoAddress); // Set up app contexts

    this.appContextPool = new _apps.default();
  ...
  async init(options = {}) {
    ...
    this.pathIntents = new _rxjs.Subject();
    this.transactions = new _rxjs.Subject();
    this.signatures = new _rxjs.Subject();
  }
  /**
  ...
  async initAccounts({
    ...
    this.accounts = new _rxjs.ReplaySubject(1);
    const accounts = fetchFromWeb3 ? providedAccounts.concat(await this.web3.eth.getAccounts()) : providedAccounts;
  ...
  async initAcl({
    ...
    this.aclProxy = (0, _utils.makeProxy)(aclAddress, 'ACL', this.web3, {
      initializationBlock: this.kernelProxy.initializationBlock
    });
    const SET_PERMISSION_EVENT = 'SetPermission';
    const CHANGE_PERMISSION_MANAGER_EVENT = 'ChangePermissionManager';
    const ACL_CACHE_KEY = (0, _utils.getCacheKey)(aclAddress, 'acl');
    ...
    const pastEvents$ = this.aclProxy.pastEvents(null, pastEventsOptions).pipe((0, _operators.mergeMap)(pastEvents => (0, _rxjs.from)(pastEvents)), // Custom cache event
    (0, _operators.endWith)({
      event: ACL_CACHE_KEY,
      returnValues: {}
    }));
    const currentEvents$ = this.aclProxy.events(null, {
      fromBlock: cacheBlockHeight + 1
    }).pipe((0, _operators.startWith)({
      event: 'starting current events',
      returnValues: {}
    })); // Permissions Object:
    // { app -> role -> { manager, allowedEntities -> [ entities with permission ] } }

    const fetchedPermissions$ = (0, _rxjs.concat)(pastEvents$, currentEvents$).pipe((0, _operators.scan)(([permissions], event) => {
      ...
      if (eventData.app) {
        ...
        if (event.event === SET_PERMISSION_EVENT) {
          const key = `${eventData.role}.allowedEntities`; // Converts to and from a set to avoid duplicated entities

          const allowedEntitiesSet = new Set(_dotProp.default.get(appPermissions, key, []));
          ...
          _dotProp.default.set(appPermissions, key, Array.from(allowedEntitiesSet));
        }
        ...
        if (event.event === CHANGE_PERMISSION_MANAGER_EVENT) {
          ...
          _dotProp.default.set(appPermissions, `${eventData.role}.manager`, eventData.manager);
        }
    ...
    }, [(0, _utils.makeAddressMapProxy)(cachedPermissions || {})]), // Cache if we're finished syncing up to cache block height
    (0, _operators.map)(([permissions, event]) => {
      if (event.event === ACL_CACHE_KEY) {
        this.cache.set(ACL_CACHE_KEY, // Make copy for cache
        {
    ...
    (0, _operators.debounceTime)(30), (0, _operators.publishReplay)(1));
    fetchedPermissions$.connect();
    const cachedPermissions$ = cachedPermissions ? (0, _rxjs.of)((0, _utils.makeAddressMapProxy)(cachedPermissions)) : (0, _rxjs.of)();
    this.permissions = (0, _rxjs.concat)(cachedPermissions$, fetchedPermissions$).pipe((0, _operators.publishReplay)(1));
  ...
  isKernelAddress(address) {
    return (0, _utils.addressesEqual)(address, this.kernelProxy.address);
  }
  /**
  ...
  initApps() {
     ...
     *                            *
     ******************************/
    const applicationInfoCache = new _utils.AsyncRequestCache(async cacheKey => {
      const [appId, codeAddress] = cacheKey.split('.');
      return (0, _aragonOS.getAragonOsInternalAppInfo)(appId) || (0, _apm.getApmInternalAppInfo)(appId) || this.apm.fetchLatestRepoContentForContract(await this.ens.resolve(appId), codeAddress);
    });
    const proxyContractValueCache = new _utils.AsyncRequestCache(proxyAddress => {
      if (this.isKernelAddress(proxyAddress)) {
        const kernelProxy = (0, _utils.makeProxy)(proxyAddress, 'ERCProxy', this.web3);
        return Promise.all([// Use Kernel ABI
        ...
        kernelProxy.call('implementation')]).then(values => ({
          appId: values[0],
          codeAddress: values[1]
        }));
      }

      const appProxy = (0, _utils.makeProxy)(proxyAddress, 'AppProxy', this.web3);
      const appProxyForwarder = (0, _utils.makeProxy)(proxyAddress, 'Forwarder', this.web3);
      return Promise.all([appProxy.call('kernel'), appProxy.call('appId'), appProxy.call('implementation'), // Not all apps implement the forwarding interface
      appProxyForwarder.call('isForwarder').catch(() => false)]).then(values => ({
    ...
    const installedApps$ = this.permissions.pipe((0, _operators.map)(Object.keys), // Dedupe until apps change
    (0, _operators.distinctUntilChanged)((oldProxies, newProxies) => {
    ...
    (0, _operators.map)(proxyAddresses => {
    ...
    (0, _operators.switchMap)(proxyAddresses => Promise.all(proxyAddresses.map(async proxyAddress => {
      let proxyValues;
    ...
    (0, _operators.map)(appProxies => appProxies.filter(appProxy => this.isApp(appProxy) || this.isKernelAddress(appProxy.proxyAddress)))); // SetApp events are emitted when apps are installed and upgraded
    ...
    const updatedApps$ = this.kernelProxy // Only need to subscribe from latest block
    .events('SetApp', {
      fromBlock: 'latest'
    }).pipe( // Only care about changes if they're in the APP_BASE namespace
    (0, _operators.filter)(({
      returnValues
    }) => (0, _kernel.isKernelAppCodeNamespace)(returnValues.namespace)), // Merge with latest value of installedApps$ so we can return the full list of apps
    (0, _operators.withLatestFrom)(installedApps$, function updateApps(setAppEvent, apps) {
      const {
        appId: setAppId
      } = setAppEvent.returnValues;
      return apps.map(async app => {
        ...
        try {
          proxyValues = await proxyContractValueCache.request(app.proxyAddress, true // force cache invalidation
          );
        } catch (_) {}

        return _objectSpread(_objectSpread(_objectSpread({}, app), proxyValues), {}, {
          updated: true
        });
      });
    }), // Emit resolved array of promises, one at a time
    (0, _operators.concatMap)(updatedApps => Promise.all(updatedApps))); // We merge these two observables, which both return the full list of apps attached with their
    // proxy values:
    ...
    const apps$ = (0, _rxjs.merge)(installedApps$, updatedApps$); // Get artifact info for apps

    const appsWithInfo$ = apps$.pipe((0, _operators.concatMap)(apps => Promise.all(apps.map(async app => {
      let appInfo;
      ...
      if (app.isForwarder && appInfo && Array.isArray(appInfo.abi) && !appInfo.abi.some(({
        type,
        name
      }) => type === 'function' && name === 'isForwarder')) {
        isForwarderOverride = {
          isForwarder: false
        };
      }

      return _objectSpread(_objectSpread(_objectSpread({}, appInfo), app), isForwarderOverride);
    }))));
    this.apps = appsWithInfo$.pipe((0, _operators.publishReplay)(1));
    this.apps.connect();
    /*******************************
     *                             *
     *            REPOS            *
    ...
    const repo$ = apps$.pipe( // Map installed apps into a deduped list of their aragonPM repos, with these assumptions:
    ...
    (0, _operators.map)(apps => Object.values(apps.filter(({
      appId
    }) => !(0, _aragonOS.isAragonOsInternalApp)(appId)).reduce((installedRepos, {
      appId,
      codeAddress,
      updated
    }) => {
    ...
    (0, _operators.filter)(([newRepoAppIds, updatedRepoAppIds]) => newRepoAppIds.length || updatedRepoAppIds.length), // Project new repos into their ids and web3 proxy objects
    (0, _operators.concatMap)(async ([newRepoAppIds, updatedRepoAppIds]) => {
      const newRepos = (await Promise.all(newRepoAppIds.map(async appId => {
        let repoProxy;

        try {
          const repoAddress = await this.ens.resolve(appId);
          repoProxy = (0, _repo.makeRepoProxy)(repoAddress, this.web3);
    ...
    (0, _operators.mergeMap)(([newRepos, updatedRepoAppIds]) => {
      // Create a new observable to project each new update as its own update emission.
      const update$ = (0, _rxjs.of)(...updatedRepoAppIds).pipe((0, _operators.map)(appId => ({
        appId
      }))); // Create a new observable to project each new repo as its own emission.

      const newRepo$ = (0, _rxjs.of)(...newRepos); // Create a new observable to project each new repo's address as its own update emission.

      const repoAddress$ = newRepo$.pipe((0, _operators.map)(({
        appId,
        repoProxy
      }) => ({
        appId,
      ...
      const version$ = newRepo$.pipe( // `mergeMap()` to "flatten" the async transformation. This async function returns an
      ...
      (0, _operators.mergeMap)(async ({
        ...
        const initialVersions = [// Immediately query state from the repo contract, to avoid having to wait until all
        ...
        (0, _operators.concatMap)(async event => {
          const {
            versionId: eventVersionId
          } = event.returnValues; // Adjust from Ethereum time

          const timestamp = (await this.web3.eth.getBlock(event.blockNumber)).timestamp * 1000;
          const versionIndex = initialVersions.findIndex(({
            versionId
          }) => versionId === eventVersionId);
          const versionInfo = versionIndex === -1 ? await (0, _repo.getRepoVersionById)(repoProxy, eventVersionId) : initialVersions[versionIndex];
          return _objectSpread(_objectSpread({}, versionInfo), {}, {
            timestamp
          });
        ...
        (0, _operators.scan)(({
          ...
          if (newVersionInfo) {
            const versionIndex = versions.findIndex(({
              versionId
            }) => versionId === newVersionInfo.versionId);
      ...
      (0, _operators.mergeAll)()); // Merge all of the repo update events resulting from the apps being updated, and return it
      // to the upper `mergeMap()` so it can be re-flattened into a single event stream.

      return (0, _rxjs.merge)(repoAddress$, version$, update$);
    ...
    (0, _operators.scan)(({
      ...
      const {
        appId: updatedAppId
            ...
            update = (0, _objectWithoutProperties2.default)(repoUpdate, ["appId"]);

      const updatedRepoInfo = _objectSpread(_objectSpread({}, repos[updatedAppId]), update);

      return {
        repos: _objectSpread(_objectSpread({}, repos), {}, {
    ...
    (0, _operators.filter)(({
      repos,
      updatedRepoAppId
    }) => !!updatedRepoAppId && Array.isArray(repos[updatedRepoAppId].versions)), // Grab the full information of the updated repo using its latest values.
    // With this, we've taken the basic stream of updates for events and mapped them onto their
    // full repo objects.
    (0, _operators.concatMap)(async ({
      repos,
      updatedRepoAppId: appId
    }) => {
      const {
        repoAddress,
        versions
      ...
      const fetchVersionInfo = version => {
        return applicationInfoCache.request(`${appId}.${version.contractAddress}`).catch(() => ({})).then(content => ({
          content,
      ...
      const currentVersion = Array.from(versions) // Apply reverse to find the latest version with the currently installed contract address
      ...
      if (!currentVersion) {
        ...
        return _objectSpread(_objectSpread({}, baseRepoInfo), {}, {
          currentVersion: null,
      ...
      const latestVersionInfoRequest = (0, _utils.addressesEqual)(currentVersion.contractAddress, latestVersion.contractAddress) ? currentVersionInfoRequest : fetchVersionInfo(latestVersion);
      return _objectSpread(_objectSpread({}, baseRepoInfo), {}, {
        currentVersion: await currentVersionInfoRequest,
        latestVersion: await latestVersionInfoRequest
      });
    }));
    this.installedRepos = repo$.pipe( // Finally, we reduce the merged updates from individual repos into one final, expanding array
    // of the installed repos
    (0, _operators.scan)((repos, updatedRepo) => {
    ...
    (0, _operators.throttleTime)(500, _rxjs.asyncScheduler, {
      leading: false,
      trailing: true
  ...
  initForwarders() {
    this.forwarders = this.apps.pipe((0, _operators.map)(apps => apps.filter(app => app.isForwarder)), (0, _operators.publishReplay)(1));
  ...
  initAppIdentifiers() {
    this.appIdentifiers = new _rxjs.BehaviorSubject({}).pipe((0, _operators.scan)((identifiers, {
      address,
      identifier
  ...
  async initIdentityProviders() {
    ...
    const identityProviders = [...defaultIdentityProviders, ...detectedIdentityProviders]; // Init all providers

    await Promise.all(identityProviders.map(({
      provider
    }) => {
    ...
    this.identityProviderRegistrar = new Map(identityProviders.map(({
      name,
      provider
    }) => [name, provider])); // Set up identity modification intent observable

    this.identityIntents = new _rxjs.Subject();
  }
  /**
  ...
  initGuiStyle({
    ...
    this.guiStyle = new _rxjs.BehaviorSubject({
  ...
  async initNetwork() {
    this.network = new _rxjs.ReplaySubject(1);
    this.network.next({
  ...
  setAppPath(appAddress, path) {
    ...
    this.appContextPool.emit(appAddress, _apps.APP_CONTEXTS.PATH, path);
  }
  /**
   * Run an app.
   *
  ...
  async runApp(proxyAddress) {
    ...
    const apps = await this.apps.pipe((0, _operators.first)()).toPromise();
    const app = apps.find(app => (0, _utils.addressesEqual)(app.proxyAddress, proxyAddress)); // TODO: handle undefined (no proxy found), otherwise when calling app.proxyAddress next, it will throw

    const appProxy = (0, _utils.makeProxyFromAppABI)(app.proxyAddress, app.abi, this.web3);
    await appProxy.updateInitializationBlock(); // Step 2: Associate app with running context

    return sandboxMessengerProvider => {
      // Set up messenger
      const messenger = new _rpcMessenger.default(sandboxMessengerProvider); // Wrap requests with the application proxy
      ...
      const request$ = messenger.requests().pipe((0, _operators.map)(request => ({
        request,
        proxy: appProxy,
        wrapper: this
      ...
      handlers.createRequestHandler(request$, 'identify', handlers.appIdentifier), handlers.createRequestHandler(request$, 'address_identity', handlers.addressIdentity), handlers.createRequestHandler(request$, 'search_identities', handlers.searchIdentities)).subscribe(response => messenger.sendResponse(response.id, response.payload)); // The attached unsubscribe isn't automatically bound to the subscription

      const shutdown = () => handlerSubscription.unsubscribe();

      const shutdownAndClearCache = async () => {
        shutdown(); // Remove all cache keys related to this app one by one

        return Object.keys(await this.cache.getAll()).reduce((promise, cacheKey) => {
          return promise.then(() => cacheKey.startsWith(proxyAddress) ? this.cache.remove(cacheKey) : Promise.resolve());
        }, Promise.resolve());
      };
  ...
  getAccounts() {
    return this.accounts.pipe((0, _operators.first)()).toPromise();
  }
  /**
  ...
  getApp(proxyAddress) {
    return this.apps.pipe((0, _operators.map)(apps => apps.find(app => (0, _utils.addressesEqual)(app.proxyAddress, proxyAddress))), (0, _operators.first)()).toPromise();
  }
  /**
  ...
  async getTransactionPath(destination, methodSignature, params, finalForwarder) {
    ...
    for (let account of accounts) {
      const path = await this.calculateTransactionPath(account, destination, methodSignature, params, finalForwarder);
  ...
  async getExternalTransactionPath(destination, methodJsonDescription, params) {
    if ((0, _utils.addressesEqual)(destination, this.aclProxy.address)) {
      try {
    ...
    try {
      const tx = await (0, _transactions.createDirectTransaction)(account, destination, methodJsonDescription, params, this.web3);
  ...
  async getTransactionPathForIntentBasket(intentBasket, {
    checkMode = 'all'
  } = {}) {
    // Get transaction paths for entire basket
    const intentsToCheck = checkMode === 'all' ? intentBasket // all -- use all intents
    : checkMode === 'single' ? [intentBasket[0]] // single -- only use first intent
    : [];
    const intentPaths = await Promise.all(intentsToCheck.map(([destination, methodSignature, params]) => (0, _utils.addressesEqual)(destination, this.aclProxy.address) ? this.getACLTransactionPath(methodSignature, params) : this.getTransactionPath(destination, methodSignature, params))); // If the paths don't match, we can't send the transactions in this intent basket together

    const pathsMatch = (0, _intents.doIntentPathsMatch)(intentPaths);
    ...
    if (pathsMatch) {
      ...
      const directTransactions = await Promise.all(intentBasket.map(async ([destination, methodSignature, params]) => (0, _transactions.createDirectTransactionForApp)(sender, await this.getApp(destination), methodSignature, params, this.web3)));

      if (intentPaths[0].length === 1) {
        // Sender has direct access
        try {
          const decoratedTransactions = await this.describeTransactionPath(await Promise.all(directTransactions.map(transaction => this.applyTransactionGas(transaction))));
          return {
            path: [],
        ...
        const createForwarderTransaction = (0, _transactions.createForwarderTransactionBuilder)(sender, {}, this.web3);
        const forwarderPath = intentPaths[0] // Ignore the last part of the path, which was the original intent
        .slice(0, -1) // Start from the "last" forwarder and move backwards to the sender
        .reverse() // Just use the forwarders' addresses
        .map(({
          to
        }) => to).reduce((path, nextForwarder) => {
          const lastStep = path[0];
          const encodedLastStep = (0, _callscript.encodeCallScript)(Array.isArray(lastStep) ? lastStep : [lastStep]);
  ...
  async getPermissionManager(appAddress, roleHash) {
    const permissions = await this.permissions.pipe((0, _operators.first)()).toPromise();
    const appPermissions = permissions[appAddress];
    return _dotProp.default.get(appPermissions, `${roleHash}.manager`);
  }
  /**
  ...
  async getACLTransactionPath(method, params) {
    ...
    if (functionArtifact.roles && functionArtifact.roles.length !== 0) {
      ...
      const methodABI = acl.abi.find(item => item.name === method && item.type === 'function');
  ...
  decodeTransactionPath(script) {
    ...
    if (!(0, _callscript.isCallScript)(script)) {
      throw new Error(`Script could not be decoded: ${script}`);
    }

    const path = (0, _callscript.decodeCallScript)(script);
    return path.map(segment => {
      const {
        data
      } = segment;

      if ((0, _forwarding.isValidForwardCall)(data)) {
        const forwardedEvmScript = (0, _forwarding.parseForwardCall)(data);
  ...
  async describeTransactionPath(path) {
    return Promise.all(path.map(async step => {
      ...
      if (Array.isArray(step)) {
        ...
        try {
          decoratedStep = await (0, _radspec.tryDescribingUpgradeOrganizationBasket)(step, this);
        } catch (err) {} // If the step wasn't handled, just individually describe each of the transactions
      ...
      try {
        decoratedStep = await (0, _radspec.tryDescribingUpdateAppIntent)(step, this);
      } catch (err) {} // Finally, if the step wasn't handled yet, evaluate via radspec normally


      if (!decoratedStep) {
        try {
          decoratedStep = await (0, _radspec.tryEvaluatingRadspec)(step, this);
        } catch (err) {}
      ...
      if (decoratedStep) {
        if (decoratedStep.description) {
          try {
            const processed = await (0, _radspec.postprocessRadspecDescription)(decoratedStep.description, this);
  ...
  canForward(forwarder, sender, script) {
    const canForward = new this.web3.eth.Contract((0, _interfaces.getAbi)('aragon/Forwarder'), forwarder).methods['canForward'];
  ...
  async applyTransactionGas(transaction, isForwarding = false) {
    ...
    if (transaction.pretransaction) {
      ...
      transaction.pretransaction = await this.applyTransactionGas(transaction.pretransaction, false); // Note: for transactions with pretransactions gas limit and price cannot be calculated
    ...
    const estimatedGasLimit = await this.web3.eth.estimateGas(_objectSpread(_objectSpread({}, transaction), {}, {
      gas: undefined
    }));
    const recommendedGasLimit = await (0, _transactions.getRecommendedGasLimit)(this.web3, estimatedGasLimit); // If the gas provided in the intent is lower than the estimated gas, use the estimation
  ...
  async calculateTransactionPath(sender, destination, methodSignature, params, finalForwarder) {
    ...
    const method = methods.find(method => fullMethodSignature ? method.sig === methodSignature // If the full signature isn't given, just select the first overload declared
    ...
    const finalForwarderProvided = (0, _web3Utils.isAddress)(finalForwarder);
    const directTransaction = await (0, _transactions.createDirectTransactionForApp)(sender, app, method.sig, params, this.web3); // We can already assume the user is able to directly invoke the action if:
    //   - The method has no ACL requirements and no final forwarder was given, or
    //   - The final forwarder matches the sender

    if (method.roles.length === 0 && !finalForwarderProvided || (0, _utils.addressesEqual)(finalForwarder, sender)) {
      try {
    ...
    const forwarders = await this.forwarders.pipe((0, _operators.first)()).toPromise().then(forwarders => forwarders.map(forwarder => forwarder.proxyAddress));
    let forwardersWithPermission;

    if (finalForwarderProvided) {
      if (!(0, _utils.includesAddress)(forwarders, finalForwarder)) {
      ...
      const roleSig = app.roles.find(role => role.id === method.roles[0]).bytes;

      const allowedEntities = _dotProp.default.get(destinationPermissions, `${roleSig}.allowedEntities`, []); // No one has access, so of course we don't as well
      ...
      if ((0, _utils.includesAddress)(allowedEntities, sender) || (0, _utils.includesAddress)(allowedEntities, _utils.ANY_ENTITY)) {
        try {
          // `applyTransactionGas` can throw if the transaction will fail
          return [await this.applyTransactionGas(directTransaction)];
      ...
      forwardersWithPermission = forwarders.filter(forwarder => (0, _utils.includesAddress)(allowedEntities, forwarder));
    }
  ...
  async calculateForwardingPath(sender, destination, directTransaction, forwardersWithPermission) {
    ...
    const createForwarderTransaction = (0, _transactions.createForwarderTransactionBuilder)(sender, directTransaction, this.web3); // Check if one of the forwarders that has permission to perform an action
    // with `sig` on `address` can forward for us directly

    for (const forwarder of forwardersWithPermission) {
      const script = (0, _callscript.encodeCallScript)([directTransaction]);

      if (await this.canForward(forwarder, sender, script)) {
        const transaction = createForwarderTransaction(forwarder, script);

        try {
          const transactionWithFee = await (0, _transactions.applyForwardingFeePretransaction)(transaction, this.web3); // `applyTransactionGas` can throw if the transaction will fail
    ...
    const forwarders = await this.forwarders.pipe((0, _operators.first)()).toPromise().then(forwarders => forwarders.map(forwarder => forwarder.proxyAddress).filter(forwarder => !(0, _utils.includesAddress)(forwardersWithPermission, forwarder))); // Set up the path finding queue
    ...
    const queue = forwardersWithPermission.map(forwarderWithPermission => {
      return [[createForwarderTransaction(forwarderWithPermission, (0, _callscript.encodeCallScript)([directTransaction])), directTransaction], forwarders];
    ...
    do {
      ...
      const script = (0, _callscript.encodeCallScript)([path[0]]);
